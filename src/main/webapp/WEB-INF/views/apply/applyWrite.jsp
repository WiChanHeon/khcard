...
<div class="apply_box">
 ...
 <form:form action="applywrite.do" commandName="apply" id="apply_register" name="apagree" onsubmit="return frmchk();" >
 	...
 	<span>
 	<a>
 	<button type="button" class="applyBtn" id="myBtn">동의서보기</button>
 	</a>
 	</span>
 	<div class="container">
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p>시스템 장애 또는 기타 사유로 카드 신청이 완료되지 못한 경우, 회원님의 편의를 위해 KH카드 상담원이 회원님께 전화를 드리기 위해 아래의 동의가 필요합니다. 동의를 하지 않으셔도 카드 신청은 가능합니다.
		  <p>
[개인정보 수집 이용에 대한 동의]
...
상기와 같이 회원님께 전화를 드리기 위하여, 「개인정보보호법」의 관련 규정에 따라 회원님의 주민등록번호를 수집하여 동의일로부터 일정기간 동안 보유 · 이용하기 위해 동의가 필요합니다.</p>
 	...
 	<ul class="apply_list">
 		...
 		<label for="ap_rrn">주민등록번호</label>
		   ...
		   - <input type="password" id="ap_rrnrear" name="ap_rrnrear" onkeyup="end(this);" maxlength="7"/>
			<input type="button" onclick="checks();" value="검사" class="buttonstyle" id="endRrn"/>
		   ...
		   - <form:input type="password" path="ap_rrnrear" name="ap_rrnrear" onkeyup="end(this);" maxlength="7"/>
			<input type="button" onclick="checks();" value="검사" id="endRrn"/>
			<span class="error-color"></span>
        ...
        <label for="ap_job">직업</label>  
          <form:select path="ap_job">
				...
				<form:errors path="ap_job2" class="error-color"/><hr>
        </li>
        <li class="align-center">
          <input type="submit" value="카드신청" class="applybutton">
          <input type="button" value="홈으로" onclick="location.href='${pageContext.request.contextPath}/main/main.do'" class="applybutton">
        </li>
